function [estimated_BB, qxy, Pxy]=GEN_SS_PREDICT_BB(CAM_volume, BoW, qxy, Pxy, SA_box, predict_inputs, previous_positions, previous_BB, img_size)
        ...
        bb_H_W_std_Multiplier=predict_inputs(1);
        bb_learning_ratio=predict_inputs(2);

        [centroid, moment]=calculateCentroidAndMoment(opt_CAM);
        x=[1 centroid(1) sqrt(moment(1)) centroid(2) sqrt(moment(2))];

        % Scale Estimated BB to SA
        fixed=3; % This controls the BB HW method. If 1, then BB WH do not change. If 2 BB WH are updated each frame
        if fixed==1
        ...
        if KF_switch==1
            ...
            [qxy,Pxy]=GEN_SS_KF(xy_est, previous_positions, qxy, Pxy);
