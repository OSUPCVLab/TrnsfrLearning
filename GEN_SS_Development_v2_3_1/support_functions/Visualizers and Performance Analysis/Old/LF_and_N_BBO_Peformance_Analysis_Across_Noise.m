...
function LF_and_N_BBO_Peformance_Analysis_Across_Noise(topNum_Array, LF_Array, data_folder_path1, noise1, data_folder_path2, noise2)
close all; clc;
...
for LF=LF_Array
    for topNum=topNum_Array
        ...
        bbo_mean_and_std_all_LF_n(n,1)=mean(mean_error_bbo_dist_mat(:,1));
        bbo_mean_and_std_all_LF_n(n,2)=sqrt(sum(mean_error_bbo_dist_mat(:,2).^2))/length(mean_error_bbo_dist_mat(:,2));
...
ylabel('BBO Ratio')
title(['Analysis of LF and n on BBO Ratio w/ ' num2str(noise1) ' Gaussian Noise'])
grid on
hold on
...
set(gca, 'XtickLabel',legend_string);
ylabel('BBO Ratio')
title(['Analysis of LF and n on BBO Ratio w/ ' num2str(noise2) ' Gaussian Noise'])
grid on
